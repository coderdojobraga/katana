defmodule Katana.LocationsTest do
  ...
  describe "locations" do
    ...
    test "create_location/1 with valid data creates a location" do
      valid_attrs = %{name: "some name", link: "some link"}

      assert {:ok, %Location{} = location} = Locations.create_location(valid_attrs)
      assert location.name == "some name"
      assert location.link == "some link"
    end
    ...
    test "update_location/2 with valid data updates the location" do
      location = location_fixture()
      update_attrs = %{name: "some updated name", link: "some updated link"}

      assert {:ok, %Location{} = location} = Locations.update_location(location, update_attrs)
      assert location.name == "some updated name"
      assert location.link == "some updated link"
    end
